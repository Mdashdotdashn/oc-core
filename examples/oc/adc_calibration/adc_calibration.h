...
public:
    ...
    void idle() override {
        if (pending_save_) {
            ...
            if (saved_ok_) {
                last_save_state_ = SaveState::kOk;
            } else {
                last_save_state_ = SaveState::kError;
            }
...
private:
    ...
    void draw_adc() {
        ...
        gfx_.setPrintPos(66, 22);
        print_mv(runtime_.hardware().adc()->get_calibrated(adc_channel_));
    ...
    void draw_save_status(uint8_t x) {
        ...
        if (last_save_state_ == SaveState::kOk) {
            gfx_.print("OK   ");
            return;
        }
        if (last_save_state_ == SaveState::kError) {
            gfx_.print("ERR  ");
            return;
        }
        gfx_.print("---- ");
    ...
    void print_mv(int32_t millivolts) {
        if (millivolts < 0) {
            gfx_.print("-");
        ...
        gfx_.print(static_cast<int>(millivolts / 1000));
        gfx_.print(".");
        const int frac = static_cast<int>((millivolts % 1000) / 10);
        if (frac < 10) gfx_.print("0");
        gfx_.print(frac);
        gfx_.print("V");
    }
    ...
    volatile bool pending_save_ = false;
    SaveState last_save_state_ = SaveState::kNone;
};